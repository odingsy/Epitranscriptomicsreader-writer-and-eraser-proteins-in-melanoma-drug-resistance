...
tbl <- readxl::read_excel(file.path(base, 'data', 'phenotypicAssays.xlsx'), sheet = 'shTRMU survival', range = 'R1C1:R126C9', na = '0') %>%  
...
# two other shTRMU -----
tbl <- readxl::read_excel(file.path(base, 'data', 'phenotypicAssays.xlsx'), sheet = 'shTRMU otherSeq', range = 'R1C1:R75C7', na = '0') %>%  
  dplyr::filter(timepoint == 2) %>% 
  dplyr::select(-timepoint) %>% 
...
mk3 <- drm(`relative survival` ~ nM, subset = celltype == 'shTRMU3', data = tbl, fct = LN.4()) 
...
kp3 <- cbind(newdata, predict(mk3, newdata = newdata, interval = 'confidence'), celltype = 'shTRMU2');names(kp3)[2] <- 'relative survival'
ic50_ctrl <- signif(10^(ED(mc, 50, interval = "delta")[1])/1000, digits = 3)
ic50_kd2 <- signif(10^(ED(mk2, 50, interval = "delta")[1])/1000, digits = 3)
ic50_kd3 <- signif(10^(ED(mk3, 50, interval = "delta")[1])/1000, digits = 3)
...
p <- tbl %>%
  ...
  geom_errorbar(data = kp3, aes(ymin=kp3[,'Lower'] , ymax=kp3[,'Upper'] ), linewidth = global_linewidth, width = 0.05)+
  scale_x_continuous(breaks = 0:5, labels = c('1 nM', '10 nM', '100 nM', '1 μM', '10 μM', '100 μM'))+
  scale_y_continuous(breaks = seq(0, 1, length.out = 5))+
  theme_classic()+
  coord_cartesian(ylim = c(0, 1))+
